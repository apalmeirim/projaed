...
{                                                                   
    ...
    public boolean equalKeys(OrderedDictionary<K, V> dictionary){
        if(this.size() != dictionary.size()) return false;// O(1) melhor caso
        Iterator<Entry<K,V>> it1 = this.iterator();//O(n) pior caso
        Iterator<Entry<K,V>> it2 = dictionary.iterator();
        while(it1.hasNext()){
            if(it1.next().getKey().compareTo(it2.next().getKey()) != 0) return false;
        }
        return true;
    }
    protected int subTreeSize(BSTNode<K,V> node){
        if(node == null) return 0;//O(1) melhor caso
        return 1 + subTreeSize(node.getLeft()) + subTreeSize(node.getRight()); //O(n) pior caso
    }
    private Entry<K,V>nthEntry(BSTNode<K,V> node, int n){
        int rootOrder = node.leftSubTreeSize() + 1;
        if(rootOrder == n) return node.getEntry();
        if(n < rootOrder) return nthEntry(node.getLeft(), n);
        else
            return nthEntry(node.getRight(), n-rootOrder);
    }
