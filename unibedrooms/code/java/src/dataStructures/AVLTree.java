...
{                                                                   
    ...
    protected boolean rebalanceRemRight( AVLNode<K,V> node, Stack<PathStep<K,V>> path )
    {
        AVLNode<K,V> leftChild = (AVLNode<K,V>) node.getLeft();
        switch ( leftChild.getBalance() )
        {
    ...
    protected boolean rebalanceRemLeft( AVLNode<K,V> node, Stack<PathStep<K,V>> path )
    {
        AVLNode<K,V> rightChild = (AVLNode<K,V>) node.getRight();
        switch ( rightChild.getBalance() )
        {
            ...
            if (lastStep.isLeftChild)
                ...
                switch (parent.getBalance()) {
                    ...
                    case 'E':
                        ...
                        break;
                    case 'R':
                        decrease = rebalanceRemLeft(parent, path);
                        break;
                }
            else
                // parent's right subtree has grown.
                switch (parent.getBalance()) {
                    case 'L':
                        decrease = rebalanceRemRight(parent, path);
                        break;
                    case 'E':
                        parent.setBalance('L');
                        decrease = false;
